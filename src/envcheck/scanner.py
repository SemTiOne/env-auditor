...
def _scan_file(filepath: Path, root: Path, result: ScanResult) -> None:
    ...
    if size > FILE_SIZE_LIMIT:
        ...
        print(
            f"envcheck: warning: skipping {rel} (size {size} bytes exceeds 1 MB limit)",
            file=sys.stderr,
        )
    ...
    try:
        ...
        rel = _rel(filepath, root)
        print(f"envcheck: warning: cannot read {rel}: {exc}", file=sys.stderr)
        result.skipped_files.append(rel)
        return
